...
:::  the /X option. Binary input with NULL bytes requires the /M option.
::/OPTIONS
:::
...
:::            the escape sequences specified by the /X option are available,
:::            even if the /X option has not been set. Any / literal within a
...
:::            expression FilterRegex. All escape sequences defined by /X are
:::            available to FilterRegex, even if /X has not been set.
:::
...
::/VERSION
:::
:::  JREPL.BAT version 7.7 was written by Dave Benham, and originally posted at
...
set ^"/options= /A: /APP: /B: /C: /D:":" /E: /EXC:"" /F:"" /I: /INC:""^
                ...
                /K:"" /L: /M: /MATCH: /N:0 /O:"" /OFF:0 /P:"" /PFLAG:"g" /R:"" /RTN:"" /S:""^
                /T:"none" /TFLAG:"" /U: /UTF: /V: /X: /XBYTES: /XBYTESOFF: /XREG:"" "
...
for %%F in (
  ...
  )) && certutil -f -decodehex "%%~dpnF.HEX" "%%~fF" && for %%G in (%%F) do if "%%~zG" equ "256" (
    del "%%~dpnF.HEX"
    set "/XBYTES=%%~fF"
    goto :endXBytes
  )
...
if defined /X if /i "%/T%" neq "FILE" set "/FindReplVar=1"
if defined /RTN goto :lock
if not defined /X goto :noLock
...
) else for /f "delims=" %%A in ("/!help!") do if /i "%%~pA" equ "\CharSet\" ( %= /?CHARSET/ =%
    echo(
    if defined noMore (
...
try {
  ...
  if (_g.outFileA[0]=='-') {
    if (_g.outFileA[1]===undefined) _g.outFileA[1]=_g.inFileA[1];
    _g.outFile = _g.inFileA[0]+'.new'+(_g.outFileA[1]?'|'+_g.outFileA[1]:'');
  ...
  _g.ADOStream = function( name, mode, format) {
    var that = this;
    try {
    ...
    this.Close = function() {
      if (mode!==_g.ForReading) stream.SaveToFile( name, 2 );
      stream.Close();
      stream=null;
    }
  }
  ...
  _g.openInput = function( fileName ) {
    ...
    if (fileName) {
      file = fileName.split('|');
      if (file[1]) {
        file = new _g.ADOStream( file[0], _g.ForReading, file[1] );
        return file;
      }
  ...
  _g.readVar = function( val, ref, ext ) {
    var input, buf=1024;
    if (!env('/X') || !val || env('/UTF')) return (ref && val) ? env(val) : val;
  ...
  var decode = env('/XBYTES') ?
    ...
    function(str, charSet, searchSwitch) {
      ...
      function xToUTF16(byte,charSet) {
        if (typeof _g.charMap[charSet]==='undefined') {
          if (charSet=='default' && _g.utf) {
            ...
            var stream = _g.openInput( env('/XBYTES')+(charSet=='default'?'':'|'+charSet) );
            try {
      ...
      return str.replace(
        /\\(?:\\|b|c|f|n|q|r|t|v|x([0-9a-fA-F]{2})|x{([0-9a-fA-F]{2}),([^}]+)}|u[0-9a-fA-F]{4}|u\{([0-9a-fA-F]+)\})/g,
        function($0,$1,$2,$3,$4) {
          if ($0=='\\q') return '"';
          if ($0=='\\c') return '^';
  ...
  _g.main=function() {
    ...
    var keep, reject, context;
    var rtnVar=env('/RTN');
    if (reject=env('/R')) {
      if (!/^\d+(:\d+)?$/.test(reject)) throw new Error(209, 'Invalid /R Context');
      context = reject.split(':')
      context[0]=Number(context[0]);
      context[1]=(context.length==1)?context[0]:Number(context[1]);
    }
    if (keep=env('/K')) {
      if (!/^\d+(:\d+)?$/.test(keep)) throw new Error(208, 'Invalid /K Context');
      context = keep.split(':')
      context[0]=Number(context[0]);
      context[1]=(context.length==1)?context[0]:Number(context[1]);
    }
    ...
    */
    _g.Block = function(match) {
      ...
      if (match[1]) {
        ...
        this.type='regex';
        this.spec= _g.newRegExp(decode(match[2],'input'),match[3]);
      ...
      if (match[6]) {
        ...
        if (match[7]) {
          ...
          this.endType='regex';
          this.endSpec= _g.newRegExp(decode(match[9],'input'),match[10]);
    ...
    if (translate=='none') {  // Normal
      ...
      for (var i=0; i<search.length; i++) {
        if (env('/X')) search[i]=decode(search[i],'input',literal);
        if (literal) {
        ...
        if (jquick|jmatchq) {
          if (!jexpr) {
            replace[i]="'" + (env('/X')==''?replace[i]:decode(replace[i],'output')).replace(/[\\']/g,"\\$&") + "'";
    ...
    if (filter!='') {
      ...
      filter = _g.newRegExp( decode(filter,'input'), env('/PFLAG').toLowerCase()+(env('/M')?'m':'') );
      _g.loc='';
      if (!keep&&!reject) {
        _g.search=search;
        search=filter;
...
function openOutput( fileName, append, utf ) {
  ...
  if (fileName) {
    ...
    if (file[1]) output=new _g.ADOStream( file[0], append?_g.ForAppending:_g.ForWriting, file[1] );
